...
class UserAuthentificatorAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        if($roles[0] == 'ROLE_RECRUITER') {
            return new RedirectResponse($this->urlGenerator->generate('recruiter')); 
        }
        if($roles[0] == 'ROLE_CANDIDATE') {
            return new RedirectResponse($this->urlGenerator->generate('candidate')); 
        }

        if($roles[0] == 'ROLE_CONSULTANT') {
